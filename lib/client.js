...
async function resolveLoginMethod() {
  ...
  if (forcedMethod === 'qr') {
    logger.info('LOGIN_METHOD=qr detectado. Forzando vinculación por QR...');
  ...
  if (envPhone) {
    ...
    if (cleanPhone.length >= 8) {
      logger.info(`Usando número configurado: ${cleanPhone}`);
  ...
  try {
    ...
    if (phoneNumber) {
      logger.info(`Usando el número configurado en config.js: ${phoneNumber}`);
    } else {
    ...
    logger.warn('No hay terminal interactivo disponible. Usando modo QR...');
    cachedLogin = { method: 'qr' };
    return cachedLogin;
  }
}
...
async function sendPairingCode(sock, phoneNumber) {
  try {
    ...
    logger.error('No se pudo generar el código de vinculación:', err.message || err);
  }
}
...
export async function createClient(onReady) {
  ...
  if (persistedState && persistedMemoryKeys) {
    ...
    alreadyLinked = true;
    logger.info('Reconectando con la sesión recién vinculada...');
    ...
    alreadyLinked = true;
    logger.info(`Sesión existente detectada en "${config.authFolder}", reconectando...`);
  } else {
  ...
  // (que en el fork tiene el bug de scraping que trae versiones viejas/
  // incorrectas y causa 401 al pedir el pairing code). Actualiza este
  // arreglo manualmente cuando WhatsApp deje de aceptar esta versión.
  const version = [2, 3000, 1027934701];
  logger.info(`Usando versión fija de WhatsApp Web: ${version.join('.')}`);
  ...
  sock.ev.on('creds.update', async () => {
    ...
    // Nota: aquí ya NO se persiste a disco directamente (ver fix abajo);
    // eso solo ocurre una vez la conexión abre y creds.registered es true,
    // igual que en tu bot que sí funciona.
  });
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      logger.connection('close', `code=${statusCode}, reconnect=${shouldReconnect}`);

      if (shouldReconnect) {
        const delay = getReconnectDelay();
        logger.connection('reconnect', `en ${Math.round(delay / 1000)}s`);
        setTimeout(() => createClient(onReady), delay);
      } else {
        logger.connection('logout', `Borra "${config.authFolder}" y vuelve a vincular.`);
        persistedState = null;
        persistedMemoryKeys = null;
        cachedLogin = null;
      }
    } else if (connection === 'open') {
      logger.connection('open', config.botName);
      reconnectAttempts = 0;
      mainBotSocket = sock;

      // FIX: solo persistimos a disco si la sesión ya quedó realmente
      // registrada (state.creds.registered). Antes se guardaba con solo
      // "if (memoryKeys)", lo que podía persistir una sesión a medio
      // vincular y causar reconexiones fallidas / 401 más adelante.
      if (memoryKeys && state.creds.registered) {
        try {
          ...
          persistedState = null;
          persistedMemoryKeys = null;
          logger.info('Sesión guardada en disco correctamente.');
        } catch (err) {
          logger.error('Error guardando sesión en disco:', err.message || err);
        }
      }
